...
public class MapperScannerRegistrar implements ImportBeanDefinitionRegistrar, ResourceLoaderAware {
  ...
  public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
    /**
     * 获取MapperScan类里面的所有属性，然后把MapperScan注解的属性转化为AnnotationAttributes类型(Map类型)
     */
    AnnotationAttributes mapperScanAttrs = AnnotationAttributes
        .fromMap(importingClassMetadata.getAnnotationAttributes(MapperScan.class.getName()));
    /**
     * 如果配置类上面加上了@MapperScan注解
     */
    if (mapperScanAttrs != null) {
      /**
       * 调用重写的registerBeanDefinitions
       */
  ...
  void registerBeanDefinitions(AnnotationAttributes annoAttrs, BeanDefinitionRegistry registry, String beanName) {
    /**
     * 创建Bean定义构造器，通过构造器来构建出来MapperScannerConfigurer的bean定义，使用了[建造者设计模式]
     */
    ...
    /**
     * 将MapperScannerConfigurer的Bean定义注册到容器中.
     */
